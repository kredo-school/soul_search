...
<div class="profile-container mx-auto">
    ...
    <div class="row mt-3">
        <div class="d-flex justify-content-center">
            <button onclick='crop_img()' class="btn btn-sm btn-orange me-2">Crop Image</button>
            <a type="button" href="{{ route('avatars.edit', $user->id) }}" class="btn btn-sm btn-secondary px-3">Cancel</a>
        </div>
    </div>
    ...
    <div id="crpimg">
        ...
        <div class="row mt-3">
            <div class="d-flex justify-content-center">
                <button onclick='save_img()' class="btn btn-sm btn-orange">Save Image</button>
            </div>
        </div>
    </div>
...
<script>
    ...
    let v = 1.0;   // zoom rate
    const img  = new Image();
    document.getElementById('crpimg').style.display = 'none'; // hide the cropped image area
    ...
    function crop_img(){
        ...
        document.getElementById('crpimg').style.display = ''; // show the cropped image area
    }

    function save_img(){
        const canvasData = out.toDataURL("image/png");
        const ajax = new XMLHttpRequest();
        ajax.open("POST", "{{ route('crops.update', $user->id) }}", false);
